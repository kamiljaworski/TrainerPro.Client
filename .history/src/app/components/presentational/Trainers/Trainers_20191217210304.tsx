...
import { Container, Paper, MenuItem, Button, Grid, Fade, FormControl, InputLabel, Select } from '@material-ui/core';
...
const Trainers: React.FC = () => {
    ...
    async function signToTrainer() {
        if (selected) {
            var response = await client.post("api/AssignedTrainer", { trainerId: selected });
            if (response.status === 200) {
                toast.success("You have been signed to trainer");
            }
        }
    }
                        ...
                        <FormControl className={classes.formControl}   style={{marginLeft: '12%'}}>
                            ...
                            >
                                {trainers && trainers.map(x =>
                                    <MenuItem key={x.id} value={x.id}>{x.firstName} {x.lastName}</MenuItem>
                                )}
                            </Select>
                        </FormControl>
