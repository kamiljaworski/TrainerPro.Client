...
const EditUserTrainings: React.FC<Props> = ({ username }) => {
    const [trainings, setTrainings] = useState<ITrainingDto[]>(() => {
        ...
        return days.map(x => { return { day: x as Days } as ITrainingDto });
    })
    ...
    useEffect(() => {
        (async function loadTrainings() {
            setIsLoading(true);
            const response = await client.get<ITrainingDto[]>(userTrainingsApiUrl + username);
        ...
        <Fade in={true}>
            <Paper style={{ margin: '30px auto 0 auto', boxShadow: 'none' }}>
                <MaterialTable
                    ...
                    title="Modify trainings"
                    columns={[
                        { title: 'Day', field: 'day', editable: 'never' },
                        { title: 'Training', field: 'name' },
                        { title: 'Repeats', field: 'repeats', type: 'numeric' },
                        { title: 'Series', field: 'series', type: 'numeric' },
                    ]}
                    data={trainings}
                    icons={tableIcons}
                    editable={{
