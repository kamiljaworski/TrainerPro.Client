...
const EditUserMeals: React.FC<Props> = ({ username }) => {
    const [meals, setMeals] = useState<IMealForUserDto[]>(() => {
        ...
        return days.map(x => { return { day: x as Days } as IMealForUserDto });
    })
    ...
    useEffect(() => {
        (async function loadMeals() {
            setIsLoading(true);
            const response = await client.get<IMealForUserDto[]>(userMealsApiUrl + username);
            ...
            <Paper style={{ margin: '30px auto 0 auto', boxShadow: 'none' }}>
                <MaterialTable
                    ...
                    columns={[
                        ...
                        { title: 'Breakfast', field: 'breakfastMealName' },
                        { title: 'Lunch', field: 'lunchMealName' },
                        { title: 'Dinner', field: 'dinnerMealName'},
                    ]}
                    data={meals}
                    icons={tableIcons}
                    editable={{
